...
import { MeetingRecord, AiProvider } from "./types";
import MarkdownRenderer from "./components/MarkdownRenderer";
import HistorySidebar from "./components/HistorySidebar";

// AI 服務提供商選項
const AI_PROVIDERS: { value: AiProvider; label: string; badge: string; color: string }[] = [
  {
    value: "gemini",
    label: "Google Gemini",
...
export default function App() {
  ...
  const loadRecord = (record: MeetingRecord) => {
    ...
    if (record.provider) setSelectedProvider(record.provider);
    setErrorMsg(null);
  };
        ...
        <div className="col-span-12 md:col-span-4 border-r border-gray-200 bg-white flex flex-col overflow-y-auto p-5 space-y-4" id="input-panel">
          ...
          <div className="space-y-3 pt-2 border-t border-gray-100">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2" id="ai-provider-selector">
                {AI_PROVIDERS.map((p) => (
                  <button
                    key={p.value}
                    type="button"
                  ...
                  >
                    <span className={`text-[11px] font-bold ${
                      selectedProvider === p.value
                        ? p.value === "nvidia" ? "text-green-700" : "text-indigo-700"
                        : "text-gray-700"
                    }`}>
                      {p.label}
                    </span>
                    <span className={`text-[9px] font-mono mt-0.5 ${
                      selectedProvider === p.value
                        ? p.value === "nvidia" ? "text-green-500" : "text-indigo-400"
                        : "text-gray-400"
                    }`}>
                      {p.badge}
                    </span>
                ...
                <div className="pb-4 border-b border-gray-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-mono mt-0.5">
                      GENERATED BY {selectedProvider === "nvidia" ? "NVIDIA NIM" : "GOOGLE GEMINI"} · {activeHistoryId ? `紀錄 ID: ${activeHistoryId.substring(4, 10).toUpperCase()}` : "暫存稿"}
                    </p>
                  </div>
      ...
      <footer className="h-12 bg-white border-t border-gray-200 px-6 flex items-center justify-between text-xs text-gray-400 flex-shrink-0 z-10" id="main-footer">
        <div className="flex items-center space-x-6">
          ...
          <span className="hidden sm:inline">
            背後引擎:{" "}
            {selectedProvider === "nvidia" ? (
              <span className="font-semibold text-green-600 font-mono">NVIDIA minimax-m2.7</span>
            ) : (
              <span className="font-semibold text-indigo-600 font-mono">Gemini 2.5 Flash Lite (低溫嚴謹模式)</span>
            )}
          </span>
        </div>
